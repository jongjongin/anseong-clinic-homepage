...
const DIET_THEME = guideThemes.diet;
    ...
    <div className="min-h-screen text-[#334155]" style={{ backgroundColor: DIET_THEME.tint }}>
      ...
      <main className="mx-auto max-w-5xl px-4 py-4 sm:px-6 sm:py-8">
        ...
        <GuideHero
          active="diet"
          title="다이어트, 이렇게 관리하세요"
          description="매일의 작은 습관이 목표에 가까워지는 가장 확실한 방법입니다. 아래 순서대로 함께 실천해 보세요."
        />

        <aside className="mt-3 border-l-4 bg-white px-4 py-3" style={{ borderLeftColor: DIET_THEME.accent }}>
        ...
        <section aria-labelledby="program-title" className="mt-3 border border-slate-200 bg-white p-4 sm:p-5">
            ...
            {programs.map((program) => (
              ...
              >
                <div className="flex items-center gap-3 font-bold">
                  <span>
                    <span className="text-base text-[#334155]">{program.name}</span>
                    <span className="ml-2 text-sm font-bold" style={{ color: DIET_THEME.accent }}>{program.label}</span>
                  </span>
                </div>
                <ul className="mt-2 border-t border-white/80 pt-1">
                  {program.items.map((item) => (
                    <li key={item} className="flex gap-2.5 border-b border-white/80 py-2.5 text-base leading-[1.6] text-[#334155] last:border-0">
                      <span aria-hidden="true" className="mt-2.5 h-1.5 w-1.5 shrink-0" style={{ backgroundColor: DIET_THEME.accent }} />
          ...
          <p className="mt-3 border-l-2 pl-3 text-base leading-[1.6] text-[#334155]" style={{ borderLeftColor: DIET_THEME.accent }}>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <GuideCard number="02" title="식단이 제일 중요합니다">
            <ul className="space-y-2.5 text-base leading-[1.6] text-[#334155]">
              <Bullet><strong>식사량은 평소의 절반</strong>으로 줄여 주세요.</Bullet>
              <Bullet>붓기를 줄이고 감량 속도를 높이도록 저염식으로 드세요.</Bullet>
              <Bullet>맵고 짠 자극적인 음식은 혈압과 식욕을 높일 수 있으니 피하세요.</Bullet>
            </ul>
            <p className="mt-3 px-3 py-2.5 text-base leading-[1.6] text-[#334155]" style={{ backgroundColor: DIET_THEME.tint }}>
          ...
          <GuideCard number="03" title="절대 금지식품">
            <ol className="grid grid-cols-2 gap-2">
              {prohibitedFoods.map((item) => (
                <li key={item.rank} className="px-3 py-3" style={{ backgroundColor: DIET_THEME.tint }}>
                  <span className="block text-sm font-bold" style={{ color: DIET_THEME.accent }}>{item.rank}</span>
                  <strong className="mt-0.5 block text-base text-[#334155]">{item.food}</strong>
          ...
          <GuideCard number="04" title="12시간 금식">
            <p className="break-keep text-base leading-[1.6] text-[#334155]">
              저녁 <strong>6시, 늦어도 7시 이후에는 금식</strong>해 주세요. 이 시간에 집중 감량이 일어납니다.
            </p>
            <p className="mt-3 border-l-2 pl-3 text-base leading-[1.6] text-[#334155]" style={{ borderLeftColor: DIET_THEME.accent }}>
          ...
          <GuideCard number="06" title="매일 체크">
            <ul className="space-y-2.5 text-base leading-[1.6] text-[#334155]">
              ...
              <Bullet><strong>2주에 한 번 내원해 인바디 측정과 상담</strong>을 받으세요.</Bullet>
            </ul>
          </GuideCard>
          ...
          <GuideCard number="08" title="운동·스트레스" wide>
            <div className="grid gap-3 sm:grid-cols-2">
              <p className="break-keep bg-slate-50 px-3 py-3 text-base leading-[1.6] text-[#334155]">
                <strong className="block">운동</strong>
                새 운동은 식욕이 늘 수 있어 권하지 않습니다. 기존에 하던 운동은 그대로 유지하세요.
              </p>
              <p className="break-keep px-3 py-3 text-base leading-[1.6] text-[#334155]" style={{ backgroundColor: DIET_THEME.tint }}>
                <strong className="block">스트레스</strong>
  ...
  return (
    <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
      <span
        ...
        style={{ backgroundColor: DIET_THEME.accent }}
      >
        {number}
      </span>
...
function MealCard({ meal, menu }: { meal: string; menu: string }) {
  return (
    <div className="border border-slate-200 bg-white px-3 py-3">
      <strong className="text-base" style={{ color: DIET_THEME.accent }}>{meal}</strong>
      <p className="mt-1 break-keep text-base leading-[1.6] text-[#334155]">{menu}</p>
    </div>
  );
}
